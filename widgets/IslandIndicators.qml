...
BarWidget {
  ...
  // Island geometry from the host bar when it offers it; fallbacks keep this
  // widget renderable under any other bar.
  ...
  readonly property int islandStep: islandPad * 2 + islandGap
  ...
  // Taken from the laid-out Row/Column so the inter-island spacing is counted;
  // summing the two blocks by hand would undercount it.
  ...
  Row {
    ...
    Item {
      ...
      // The clip exists to hide this area while it is collapsed. Once revealed
      // it must not clip, or it shaves the islands that overhang the first and
      // last indicator -- and padding the area instead would make this widget
      // report more width than its islands occupy, which the bar would then
      // add its own spacing on top of.
      ...
      clip: !root.revealInactiveIndicators
  ...
  component IndicatorLoader: Item {
    ...
    BorderSurface {
      z: -1
      visible: indicatorSlot.implicitWidth > 0 && indicatorSlot.implicitHeight > 0
      anchors.centerIn: parent

...
Item {
  ...
  readonly property int islandEdgeMargin: Style.space(8)   // gap to screen edge
  readonly property int islandPadX: Style.space(8)         // padding inside an island, along the bar
  readonly property int islandInset: Style.space(2)        // shrink island off the bar's cross axis
  readonly property int islandThickness: Math.max(1, root.barSize - islandInset * 2)
  readonly property int islandRadius: Style.cornerRadius
  readonly property int islandGap: Style.space(4)              // visible gap between two islands
  readonly property int islandStep: islandPadX * 2 + islandGap // slot-to-slot spacing that yields it

  // Widget settings this bar forces regardless of shell.json. A collapsed
  // widget paints no island, so the indicators keep their reveal open here;
  // other bars still follow the shared shell.json value.
  readonly property var widgetSettingOverrides: ({
  ...
  function localWidgetSource(moduleName) {
    ...
    return rel ? Qt.resolvedUrl(rel) : ""
  }

  function drawsOwnIslands(moduleName) {
    return root.selfIslandWidgets[root.canonicalWidgetId(moduleName)] === true
  }

  function settingsFor(moduleName, settings) {
    var extra = root.widgetSettingOverrides[root.canonicalWidgetId(moduleName)]
    if (!extra) return settings
    var merged = {}
    for (var k in settings) merged[k] = settings[k]
    for (var o in extra) merged[o] = extra[o]
    return merged
  }
  ...
  function applySettingsDelta(delta) {
    for (var i = 0; i < delta.length; i++) {
      ...
      for (var s = 0; s < moduleSlots.length; s++) {
        ...
        if (item && "settings" in item) item.settings = settingsFor(slot.moduleName, settings)
      }
    }
  }
  ...
  component BarPanel: PanelWindow {
    ...
    Component {
      ...
      Item {
        ...
        // Each slot paints its own island, so the sections are plain lists.
        // The edge margin gains islandPadX because the first island overhangs
        // its slot by that much.
        LeftModules {
          anchors.left: parent.left
          anchors.leftMargin: root.islandEdgeMargin + root.islandPadX
          anchors.verticalCenter: parent.verticalCenter
        }

        RightModules {
          anchors.right: parent.right
          anchors.rightMargin: root.islandEdgeMargin + root.islandPadX
          anchors.verticalCenter: parent.verticalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        LeftModules {
          ...
          anchors.topMargin: root.islandEdgeMargin + root.islandPadX
          anchors.horizontalCenter: parent.horizontalCenter
        }

        RightModules {
          anchors.bottom: parent.bottom
          anchors.bottomMargin: root.islandEdgeMargin + root.islandPadX
          anchors.horizontalCenter: parent.horizontalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        ModuleList {
          ...
          anchors.rightMargin: root.islandStep
        ...
        ModuleList {
          ...
          anchors.leftMargin: root.islandStep
          anchors.verticalCenter: centerAnchorModule.verticalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        ModuleList {
          ...
          anchors.bottomMargin: root.islandStep
        ...
        ModuleList {
          ...
          anchors.topMargin: root.islandStep
          anchors.horizontalCenter: centerAnchorModule.horizontalCenter
        }
      }
    }
  ...
  component ModuleSlot: Item {
    ...
    readonly property var registryComponent: {
      ...
      if (!w) return null
      if (customType || localCustom) return null
      var registryName = root.canonicalWidgetId(moduleName)
      return w[registryName] ? w[registryName].component : null
    }
    readonly property string localSource: root.localWidgetSource(moduleName)
    readonly property bool localCustom: !customType && localSource !== ""
    ...
    readonly property var activeItem: {
      ...
      if (qmlCustom || localCustom) return qmlLoader.item
    ...
    // One island per widget. It overhangs the slot by islandPadX on each side
    // along the bar; ModuleList spacing is sized to leave islandGap showing
    // between neighbours.
    BorderSurface {
      z: -1
    ...
    Loader {
      id: componentLoader
      active: !slot.qmlCustom && !slot.localCustom && !slot.registered
    ...
    Loader {
      id: qmlLoader
      active: slot.qmlCustom || slot.localCustom
      source: slot.qmlCustom ? root.customModuleSource(slot.entry)
                             : (slot.localCustom ? slot.localSource : "")
      anchors.fill: parent
      opacity: slot.dragSource ? 0.22 : 1.0
      onLoaded: {
        slot.injectProps()
    ...
    function injectProps() {
      ...
      if ("settings" in target) target.settings = root.settingsFor(moduleName, moduleSettings)
    }
